      ...
      <div className="max-w-5xl space-y-5">
        ...
        <div className="grid lg:grid-cols-[1fr_330px] gap-5">
          <div className="panel p-6">
            <div className="flex gap-3">
              <button onClick={() => update({ status: t.status === 'done' ? 'todo' : 'done' })}>
                {t.status === 'done' ? (
                  <CheckCircle2 size={23} style={{ color: 'var(--accent)' }} />
                ) : (
                  <Circle size={23} className="muted" />
                )}
              </button>
            ...
            <div className="mt-8 pt-5 border-t" style={{ borderColor: 'var(--border)' }}>
              ...
              <div className="space-y-4 my-5">
                {c.map((x) => (
                  <div key={x._id}>
                    <div className="font-semibold text-sm">{x.authorName || 'User'}</div>
                    <div className="muted text-sm mt-1">{x.text}</div>
                  </div>
                ))}
          ...
          <aside className="panel p-5 space-y-5">
            ...
            <button className="btn w-full" onClick={del}>
              <Trash2 size={16} />
              Delete task
            </button>
          </aside>
